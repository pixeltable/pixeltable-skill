...
└── references/                 # Loaded on demand by Claude Code / Cursor
    ...
    ├── cli.md                  # pxt CLI -- init, schema, service, db, secret, inspect
...
- Video frame search: `--template video-search` -> `uv sync` -> `pxt schema update app.py my_app` -> `pxt service update app.py my_app` (see `/pixeltable:scaffold`)
